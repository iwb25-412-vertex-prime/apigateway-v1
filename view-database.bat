...
if not exist "nodejs-backend\database\userportal.db" (
    echo Database file not found. Make sure the backend has been started at least once.
    pause
    exit /b 1
)

echo Database location: nodejs-backend\database\userportal.db
echo.
...
if %errorlevel% equ 0 (
    ...
    echo.
    sqlite3 "nodejs-backend\database\userportal.db"
) else (
    ...
    echo.
    echo Database file location: %cd%\nodejs-backend\database\userportal.db
)
