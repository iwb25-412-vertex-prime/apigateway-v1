...
for /f "tokens=*" %%i in ('curl -s -X POST %API_URL%/auth/login -H "Content-Type: application/json" -d "{\"username\": \"testuser\", \"password\": \"password123\"}" ^| jq -r .token') do set JWT_TOKEN=%%i

echo Token: %JWT_TOKEN%

echo.
echo 3. Creating API key with initial rules...
for /f "tokens=*" %%i in ('curl -s -X POST %API_URL%/apikeys -H "Content-Type: application/json" -H "Authorization: Bearer %JWT_TOKEN%" -d "{\"name\": \"Test Key\", \"description\": \"For testing rules update\", \"rules\": [\"no-spam-content\", \"no-adult-content\"]}" ^| jq -r .apiKey.id') do set API_KEY_ID=%%i
